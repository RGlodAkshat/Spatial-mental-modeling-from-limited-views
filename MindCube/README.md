...
- Creates a deterministic subset (`50 train + 50 tinybench`)
- Runs scaffold generation (`full_pipeline`)
- Generates prompts for `plain_cgmap_ffr_out`
- Converts training prompts to Qwen SFT format
- Sets up Qwen patching for MindCube datasets
- Tracks run metadata under `runs/phase_a/`
- Skips training/eval automatically on CPU-only machines
...
# Faster reruns after first setup
BOOTSTRAP_SKIP_INSTALL=true bash scripts/phase_a/run_phase_a_smoke.sh
```

### Key artifacts

- Deterministic manifest: `data/manifests/phase_a_deterministic_50_50_manifest.json`
- Full canonical SFT train file: `data/prompts/training/qwen2.5vl/MindCube_train_plain_cgmap_ffr_out_qwen_sft.json`
- Smoke SFT train file: `data/prompts/training/qwen2.5vl/MindCube_train_plain_cgmap_ffr_out_smoke_qwen_sft.json`
- Smoke tinybench prompt: `data/prompts/general/MindCube_tinybench_plain_cgmap_ffr_out_smoke.jsonl`
- Full canonical tinybench prompt (only when `PHASE_A_WRITE_FULL_CANONICAL=true`): `data/prompts/general/MindCube_tinybench_plain_cgmap_ffr_out.jsonl`
- Run logs/metadata: `runs/phase_a/`

### GPU follow-up commands

```bash
# 1) Smoke SFT with checkpointing every step
bash experiments/sft/train_qwen_sft.sh experiments/sft/config_plain_cgmap_ffr_out_smoke.sh

# 2) Evaluate smoke checkpoints
python scripts/phase_a/eval_checkpoints.py --task plain_cgmap_ffr_out_smoke --checkpoints-root checkpoints/sft/smoke/plain_cgmap_ffr_out_smoke --input-file data/prompts/general/MindCube_tinybench_plain_cgmap_ffr_out_smoke.jsonl --results-dir data/results/sft/smoke/plain_cgmap_ffr_out_smoke --eval-dir data/evaluate/sft/smoke/plain_cgmap_ffr_out_smoke
```
